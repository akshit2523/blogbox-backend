...
const getBlogs = async (req, res) => {
  try {
    const blogs = await BlogStore.find().sort({ createdOn: -1 }).lean();
...
const getBlogById = async (req, res) => {
  try {
    ...
    const blog = await BlogStore.findOne({ slug }).lean();
    ...
    sendError(res, "Error while fetching blog.", error);
  }
};
...
const editBlog = async (req, res) => {
  try {
    ...
    const blog = await BlogStore.findByIdAndUpdate(id, blogInfo, {
      new: true,
      runValidators: true,
    });
...
const filterBlog = async (req, res) => {
  try {
    ...
    const blogs = await BlogStore.find(filterQuery)
      .sort({ createdOn: -1 })
      .skip(skip)
      .limit(limitNumber);
...
const filterBlogByCategory = async (req, res) => {
  try {
    const { category, page = 1, limit = 10 } = req.body;

    if (!category) {
      return sendError(res, "Filter category not available.", null, 400);
    }
    ...
    const blogs = await BlogStore.find(filter)
      .sort({ createdOn: -1 })
      .skip(skip)
      .limit(limitNumber);
